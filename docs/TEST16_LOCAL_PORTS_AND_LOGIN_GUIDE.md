# Test16 本地端口区分、打开方式与复验步骤

> 根目录：`smart-task-board-test16`。先确认进程工作目录，再确认端口。
> 本轮只修改后端重放返回，数据库结构和小程序代码未变。
...
.venv/bin/python scripts/verify-test16-scope.py
```

尚未建立项目venv时，可以用明确可用的Python执行只读范围校验；正式门禁必须使用3.12。
不要只更新小程序：重放修复在后端，必须从Test16目录重启后端。
...
小程序文件与Test15完全相同。可复跑此前已通过的高管/员工双向门禁。
