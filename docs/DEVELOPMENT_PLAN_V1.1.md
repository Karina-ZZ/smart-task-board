...
| 管理员 | 默认可查看全部任务及任务范围内只读关联数据 | 系统配置/授权管理（若本期页面覆盖）；业务写动作仅按实际任务关系获得 | 默认代替任务角色执行接受、汇报、验收等业务写动作 |
...
**P0用户确认补充**：执行本任务前必须完整读取 `docs/FEATURE_13_NOTIFICATION_RULES.md`。AI拆解成功本身不向创建人/主承办人发送纯知悉提醒；AI拆解出的协办节点只向对应协办人发送“节点待承接”。主承办人本人节点无需再次承接；协办节点只有在本人明确确认承接后才进入正常执行提醒流程。未承接协办节点禁止发送开始、临期、到期、逾期等执行类提醒。

节点临期提醒使用动态工作时段算法，而不是固定提前24小时：节点有效工作跨度 `<=1个工作日` 时截止前 `2个工作小时`；`>1且<=3个工作日` 时截止前 `4个工作小时`；`>3个工作日` 时截止前 `1个工作日`。临期时点不得早于 `planned_start_time`，并与同一时点的开始提醒去重。计算不得使用任何 `estimated_hours`。

**实现**：通知筛选、跳任务、真实read策略、提醒/outbox重试、个人资料/摘要；角色切换和重置仅测试构建或删除；清理localStorage业务写；实现协办节点承接门禁和节点提醒动态临期算法。协办节点承接状态必须服务端持久化；若现有表无法承载，使用新增Alembic迁移，不得用前端/localStorage模拟。

**测试**：通知归属、重复通知去重、发送失败重试、无权任务跳转、生产无演示入口、仓库无generic假成功事件、localStorage仅保留UI偏好；主承办人节点免确认、协办节点待承接通知、未承接禁止执行提醒、承接后才允许执行提醒、临期三档边界、临期不早于开始、完成/取消/失效节点旧提醒不发送、estimated_hours完全不参与提醒时间计算。
...
| DEV-07 | DONE | `POST /api/v1/task-inputs/{input_id}/extract` retry endpoint; `GET /api/v1/task-inputs/{input_id}/extraction` polling endpoint; task input length/voice transcript validation; AI extraction result schema whitelist; employee/date/enum validation; `/create/details` DEV-07 intake page with text, browser speech recognition fallback, clarification, retry and session draft restore | DEV-07 backend targeted 19 passed; DEV-07 frontend targeted 43 passed; DEV-07 Playwright 18 passed | Frontend 109 passed; Playwright full 87 passed; backend pytest 368 passed/21 skipped; build/lint passed; ruff/pip check/OpenAPI passed | 2026-09-02 | No formal task write, no task send, no nodes/dependencies/effective_at writes, no decomposition, no real AI in automated tests, no frontend AI key, no hard-coded employee, no DB migration |
| DEV-08 | TODO | — | — | — | — | — |
| DEV-09 | TODO | — | — | — | — | — |
| DEV-10 | TODO | — | — | — | — | — |
| DEV-11 | TODO | — | — | — | — | — |
...
- 状态：DONE
- Baseline HEAD：`ed9fd936bc166b878ed9f5428c79fc83a5c93d2f`
- 实际文件：`app/api/v1/business.py`、`app/schemas/business.py`、`app/schemas/__init__.py`、`app/services/business_capabilities.py`、`tests/api/test_business_routes.py`、`tests/api/test_task_routes.py`、`tests/services/test_business_capabilities.py`、`web/src/api/types.ts`、`web/src/api/endpoints.ts`、`web/src/api/client.test.ts`、`web/src/app/router.tsx`、`web/src/app/router.test.tsx`、`web/src/features/task-intake/`、`web/src/features/workbench/WorkbenchPage.tsx`、`web/src/features/workbench/__tests__/WorkbenchPage.test.tsx`、`web/e2e/dev-03-workbench.spec.ts`、`web/e2e/dev-07-task-intake.spec.ts`、`docs/DEVELOPMENT_PLAN_V1.1.md`
- 迁移版本：无；复用已有 `task_inputs` 和 `ai_extraction_records`。
- API/DTO：`POST /api/v1/task-inputs` 继续保存文字/语音转写输入并触发识别；新增 `POST /api/v1/task-inputs/{input_id}/extract` 用于拥有者安全重试；新增 `GET /api/v1/task-inputs/{input_id}/extraction` 用于统一轮询最新识别结果；响应包含结构化字段、缺失项、低置信项、追问问题、job 状态和重试提示，不暴露 ORM 或 Secret。
- 后端实现：复用 `TaskIntakeService`、`FakeASRProvider`、`FakeTaskExtractionProvider` 和既有 AI provider 抽象；输入文本限制 4000 字；语音可用浏览器转写文本，不强制持久化原始音频；AI 输出只允许正式字段集合并过滤 `estimated_hours`；人员字段只能解析为已存在且唯一的活跃员工号；模糊日期、未知枚举、非法权重进入追问/低置信路径。
- 前端实现：`/create/details` 接入 `TaskIntakePage`；支持任务原文输入、浏览器 SpeechRecognition/回退、加载、60 秒提示、识别结果、缺失/低置信展示、追问补充、重试和 sessionStorage 草稿恢复；不调用正式任务创建、发送、节点或拆解 API。
- 测试结果：`ALLOW_TEST_EMPLOYEE_HEADER=true AUTH_MODE=test_header .venv/bin/python -m pytest -q tests/api/test_business_routes.py tests/services/test_business_capabilities.py tests/api/test_task_routes.py::test_openapi_and_swagger_expose_only_approved_contract` 19 passed；`npm --prefix web run test -- --run src/features/task-intake/__tests__/TaskIntakePage.test.tsx src/api/client.test.ts src/app/router.test.tsx src/features/workbench/__tests__/WorkbenchPage.test.tsx` 43 passed；`npm exec playwright test e2e/dev-07-task-intake.spec.ts` in `web/` 18 passed；`npm --prefix web run test -- --run` 109 passed；`npm --prefix web run lint` passed；`npm --prefix web run build` passed；`npm exec playwright test` in `web/` 87 passed；`ALLOW_TEST_EMPLOYEE_HEADER=true AUTH_MODE=test_header .venv/bin/python -m pytest -q` 368 passed / 21 skipped；`.venv/bin/python -m ruff check app tests` passed；`.venv/bin/python -m pip check` passed；OpenAPI passed with 81 API paths and 87 operations.
- 响应式/可访问性：Playwright 覆盖 375×812、390×844、430×932，并在 DEV-07 场景内额外检查 768×1024、1440×900；horizontal overflow <= 0；可见按钮 touch target >= 44px；任务原文、语音按钮、补充说明、状态、错误、重试和焦点回退均有可访问语义。
- 安全/权限证据：`POST /task-inputs/{input_id}/extract` 和 `GET /task-inputs/{input_id}/extraction` 均校验输入归属；前端只消费 DEV-06 `/me` 权限投影；AI_API_KEY 不进入前端；自动化测试使用 deterministic fake provider；provider 429/500/timeout/无效响应返回安全错误；prompt injection 不可改变权限、写库或字段白名单。
- 边界结果：DEV-08 task creation write = No；task send = No；formal task INSERT = No；task_nodes write = No；dependencies write = No；effective_at write = No；DEV-09 decomposition = No；node execution = No；progress writes = No；lifecycle writes = No；review/archive writes = No；calculation changes = No；real AI automated tests = No；fake AI success = No；hard-coded employee = No；secret exposure = No；localStorage business write = No；DB migration = No；`/create/nodes` restored = No
- 遗留风险：DEV-07 只产出可确认的 AI 字段识别 proposal；创建人信息确认、保存正式草稿、确认发送和正式任务落库仍按 DEV-08 执行。生产语音服务仍依赖浏览器能力或后续正式 ASR 配置；失败路径已保留文字输入回退。
