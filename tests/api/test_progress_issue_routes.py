...
def test_submit_progress_report_builds_versioned_actor_command(api_context) -> None:
    ...
    response = client.post(
        ...
        headers={"X-Employee-No": "ASSIGNEE", "Idempotency-Key": "report-123"},
        json={
            "expected_task_version": 5,
            "progress_percent": 40,
            "stage_result": "Progress",
            "has_issue": False,
            "remark": "Weekly update",
        },
    )
    ...
    assert command.report_content == "Weekly update"
    assert command.has_issue is False
    assert command.idempotency_key == "report-123"
