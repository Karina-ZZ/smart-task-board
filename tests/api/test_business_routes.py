...
def test_ai_confirmation_metric_and_planning_routes_forward_identifiers(
    ...
    services["metrics"].suggest_matches.assert_called_once_with(
        "E-ACTOR", task_id, 1, expected_task_version=1
    )
...
def test_feature12_performance_confirmation_and_clear_require_task_version(
    business_context,
) -> None:
    client, services = business_context
    task_id = uuid4()
    ...
    match_id = uuid4()
    row = SimpleNamespace(
        performance_match_id=match_id, task_id=task_id, metric_id=metric_id,
        type_score=Decimal("100"), business_unit_score=Decimal("100"),
        metric_name_score=Decimal("90"), definition_formula_score=Decimal("80"),
        deliverable_score=Decimal("60"), total_score=Decimal("88.5"),
        match_level="strong", match_reason="系统确定性相似度计算",
        is_confirmed=True, confirmed_by_employee_no="E-ACTOR", confirmed_at=NOW,
        algorithm_version="tfidf-char-ngram-v1", created_at=NOW, updated_at=NOW,
        metric_type="sales", period="2026", business_unit="宠物事业部",
        metric_name="新品销售额达成率", definition_formula="销售额/目标销售额",
    )
    services["metrics"].confirm_match.return_value = row
    services["metrics"].clear_confirmation.return_value = [row]

    missing_version = client.post(
        f"/api/v1/tasks/{task_id}/performance-matches/{match_id}/confirm",
        headers={"X-Employee-No": "E-ACTOR"}, json={},
    )
    confirmed = client.post(
        f"/api/v1/tasks/{task_id}/performance-matches/{match_id}/confirm",
        headers={"X-Employee-No": "E-ACTOR"}, json={"expectedTaskVersion": 7},
    )
    cleared = client.post(
        f"/api/v1/tasks/{task_id}/performance-matches/clear",
        headers={"X-Employee-No": "E-ACTOR"}, json={"expectedTaskVersion": 7},
    )
    ...
    services["metrics"].confirm_match.assert_called_once_with(
        "E-ACTOR", task_id, match_id, expected_task_version=7
    )
    services["metrics"].clear_confirmation.assert_called_once_with(
        "E-ACTOR", task_id, expected_task_version=7
    )
    assert confirmed.json()["metric_name"] == "新品销售额达成率"
...
def test_feature05_cloud_registration_and_external_extraction_routes(business_context) -> None:
    client, services = business_context
    input_id = uuid4()
    extraction_id = uuid4()
    services["intake"].register_input.return_value = SimpleNamespace(
        input_id=input_id,
        input_type="text",
        raw_text="周五完成渠道月报",
        asr_text=None,
    ...
    services["intake"].record_external_extraction.return_value = _intake_result(
        input_id,
        extraction_id,
    )
    ...
    recorded = client.post(
        ...
        headers={"X-Employee-No": "E-ACTOR"},
        json={
            "task_draft": {"taskName": "完成渠道月报"},
            "missing_fields": ["reviewerEmployeeNo"],
            "low_confidence_fields": [],
            "confirm_questions": ["由谁验收？"],
            "confidence_score": "0.75",
            "agent_result": {"provider": "qwen"},
        },
    )

    assert registered.status_code == 201
    assert registered.json()["input_id"] == str(input_id)
    assert recorded.status_code == 201
    assert recorded.json()["extraction_id"] == str(extraction_id)
    services["intake"].register_input.assert_called_once()
    services["intake"].record_external_extraction.assert_called_once()
