...
def route_context() -> Iterator[tuple[TestClient, MagicMock, MagicMock, MagicMock]]:
    ...
    query.list_operation_logs.return_value = {
        "items": [],
        "limit": 50,
        "offset": 0,
        "total": 0,
...
def test_create_route_builds_command_from_header(route_context) -> None:
    ...
    assert command.nodes == () and command.dependencies == ()
    assert command.participants[0].employee_no == "E-COLLAB"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("confirm-self-assigned", "confirm_self_assigned", "pending_acceptance", 3),
        ("accept", "accept_task", "decomposing", 4),
...
def test_task_action_routes_forward_actor_version_and_fixed_source(
    ...
    expected = [task_id, "E-ACTOR", version - 1, "rest_api"]
    if method_name == "confirm_and_send":
        expected.append(None)
    getattr(tasks, method_name).assert_called_once_with(*expected)
...
def test_completion_routes_forward_content_round_and_rework_scope(route_context) -> None:
    ...
    headers = {
        "X-Employee-No": "E-ACTOR",
        "Idempotency-Key": "feature11-completion-submit",
    }
...
def test_approve_completion_requires_and_forwards_review_id(route_context) -> None:
    ...
    response = client.post(
        ...
        headers={
            "X-Employee-No": "E-REVIEWER",
            "Idempotency-Key": "feature11-review-approve",
        },
        json={
            "expected_task_version": 10,
            "completion_review_id": str(review_id),
...
def test_extended_lifecycle_routes_forward_reasons_and_targets(route_context) -> None:
    ...
    tasks.reassign_task.return_value = _task_result(task_id, "pending_acceptance", 5)
    ...
    reassign = client.put(
        ...
        json={
            "expected_task_version": 4,
            "new_assignee_employee_no": "E-NEW",
    ...
    tasks.cancel_task.assert_called_once_with(task_id, "E-ACTOR", 4, "rest_api", "Duplicate", None)
    ...
    tasks.reassign_task.assert_called_once_with(
        task_id,
        "E-ACTOR",
        4,
        "rest_api",
...
def test_query_routes_forward_actor_and_pagination(route_context) -> None:
    ...
    operation_logs = client.get(
        f"/api/v1/tasks/{task_id}/operation-logs?limit=10&offset=5",
        headers=headers,
    )
    review_id = uuid4()
    ...
    query.list_operation_logs.assert_called_once_with(
        task_id,
        "E-READER",
        limit=10,
        offset=5,
...
def test_openapi_and_swagger_expose_only_approved_contract(route_context) -> None:
    ...
    wave2_operations = {
        ...
        ("PUT", "/api/v1/tasks/{task_id}/assignee"),
        ...
        ("GET", "/api/v1/tasks/{task_id}/decomposition"),
    ...
    business_operations = {
        ...
        ("POST", "/api/v1/task-inputs/register"),
        ("POST", "/api/v1/task-inputs/{input_id}/external-extractions"),
        ...
        ("POST", "/api/v1/tasks/{task_id}/performance-matches/clear"),
