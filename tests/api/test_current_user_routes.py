...
def test_current_user_uses_bearer_subject_and_returns_department(monkeypatch) -> None:
    ...
    assert response.json() == {
        ...
        "scopes": [
            {
                "authorized_scope_id": str(scope_id),
                "scope_type": "department",
                "scope_id": str(department_id),
                "permission_type": "view",
            }
    ...
    service.current_user_context.assert_called_once_with("E-CREATOR")
...
def test_current_user_rejects_unknown_or_disabled_identity(monkeypatch) -> None:
    ...
    monkeypatch.setattr(dependencies, "get_settings", lambda: settings)
    service = MagicMock()
    service.current_user_context.side_effect = PermissionDeniedError(
