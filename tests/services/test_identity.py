...
from app.core.security import InvalidPrototypeTokenError, create_access_token, decode_access_token
...
def test_current_user_permissions_are_backend_projection() -> None:
    ...
    assert service.current_user_permissions(admin, [])["can_access_executive"] is False
    assert service.current_user_permissions(employee, [])["can_view_all_tasks"] is False
    assert service.current_user_permissions(executive, [])["can_view_all_tasks"] is False
    assert service.current_user_permissions(admin, [])["can_view_all_tasks"] is True
    assert "task:read:all" in service.current_user_permissions(admin, [])["capabilities"]
    assert service.current_user_permissions(admin, [department_scope])[
        "can_access_executive"
    ] is True
    assert service.current_user_permissions(admin, [role_scope])[
        "can_access_executive"
    ] is True
    assert service.current_user_permissions(admin, [all_data_scope])[
        "can_view_all_demo_data"
    ] is True
