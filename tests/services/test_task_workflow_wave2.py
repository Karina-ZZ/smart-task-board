...
def test_creator_cancel_during_decomposition_invalidates_running_attempt() -> None:
    ...
    uow.task_decompositions.get_active_for_task.return_value = attempt

    cancelled = service.cancel_task(task.task_id, "CREATOR", 4, "unit-test", "Stop now")

    assert cancelled.status == "cancelled"
    assert attempt.status == "invalidated"
    assert attempt.error_code == "DECOMPOSITION_INVALIDATED"
    assert task.decomposition_status == "invalidated"
    assert task.effective_at is None
    uow.commit.assert_called_once_with()


def test_reassign_invalidates_decomposition_resets_acceptance_and_notifies_both_assignees() -> None:
    task = _task(status="decomposing", version=4)
    task.decomposition_status = "running"
    task.latest_decomposition_id = uuid4()
    task.effective_at = None
    service, uow = _context(task)
    attempt = MagicMock(status="running")
    uow.task_decompositions.get_active_for_task.return_value = attempt

    result = service.reassign_task(
        task.task_id,
        "CREATOR",
        4,
        "unit-test",
    ...
    assert result.main_assignee_employee_no == "NEW-ASSIGNEE"
    assert result.accepted_at is None
    assert result.effective_at is None
    assert result.latest_decomposition_id is None
    assert attempt.status == "invalidated"
    participant = uow.tasks.list_participants.return_value[0]
    assert participant.employee_no == "NEW-ASSIGNEE"
    assert participant.confirm_status == "pending"
    notifications = [
        call.args[0]
        for call in uow.session.add.call_args_list
        if hasattr(call.args[0], "recipient_employee_no")
    ]
    assert {item.recipient_employee_no for item in notifications} == {
        "ASSIGNEE",
        "NEW-ASSIGNEE",
    }


@pytest.mark.parametrize("field", ["estimated_hours", "actual_hours", "main_assignee_employee_no"])
def test_change_request_cannot_smuggle_prohibited_fields(field: str) -> None:
    task = _task()
    service, _ = _context(task)
    with pytest.raises(BusinessValidationError, match="cannot be changed"):
