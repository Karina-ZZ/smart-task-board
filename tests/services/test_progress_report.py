...
def test_task_report_is_append_only_versioned_and_periodized() -> None:
    ...
    report = service.submit(_command(task))
    ...
    assert report.actual_hours is None
...
def test_node_owner_and_collaborator_cannot_submit_task_level_or_node_reports() -> None:
    node = TaskNode(
        node_id=uuid4(),
        task_id=uuid4(),
        node_order=1,
    ...
    with pytest.raises(PermissionDeniedError):
        service.submit(_command(task, reporter_employee_no="COLLAB"))
...
def test_correction_requires_same_reporter() -> None:
    service, uow, task = _context()
    root = TaskProgressReport(
        progress_report_id=uuid4(),
        task_id=task.task_id,
