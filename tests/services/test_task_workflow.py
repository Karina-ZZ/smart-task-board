...
def test_self_assigned_confirmation_still_waits_for_acceptance() -> None:
    task = _task(
...
def test_approve_completion_uses_snapshot_reviewer_and_decides_once() -> None:
    ...
    assert archive.actual_hours_total == task.actual_hours
    logs = [call.args[0] for call in uow.task_status_logs.add.call_args_list]
    assert [(log.action_type, log.task_version) for log in logs[-2:]] == [
        ("completion_approved", 10),
        ("task_archived", 11),
    ]
    assert (logs[-1].business_ref_type, logs[-1].business_ref_id) == (
        "completion_review",
        review.completion_review_id,
    )
