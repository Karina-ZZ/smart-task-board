...
def _create_payload(refs: ApiReferences) -> dict[str, object]:
    """Build a V1.1 creator payload with task-level facts only."""
    return {
        ...
        "task_weight": 3,
        "deliverable": "Verified API workflow",
        "acceptance_criteria": "All active AI-generated nodes completed",
        "participants": [
            {"employee_no": refs.participant, "participant_role": "collaborator"},
            {"employee_no": refs.node_participant, "participant_role": "collaborator"},
        ],
...
def test_http_core_workflow_and_read_permissions(
    ...
    node_ids, active_version = complete_v11_decomposition(
        phase5_session_factory, task_id, refs.assignee, keep_active_nodes=2
    )
