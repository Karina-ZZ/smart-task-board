...
fail() { echo "[TEST10 BLOCKED] $*" >&2; exit 2; }
...
if [[ -z "${POSTGRES_TEST_DATABASE_URL:-}" ]]; then
  ...
  started_container="smart-task-board-pg-gate-test9"
  docker rm -f "$started_container" >/dev/null 2>&1 || true
  gate_password="test10-$(date +%s)-$RANDOM"
...
echo "[TEST10 AUTOMATED GATE PASS] automated gates passed."
