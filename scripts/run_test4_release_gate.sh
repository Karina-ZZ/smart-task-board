...
fail() { echo "[TEST4 BLOCKED] $*" >&2; exit 2; }
...
if [[ -z "${VIRTUAL_ENV:-}" ]]; then
  fail "activate the project Python 3.12 virtual environment before running Test4"
fi
...
if [[ -z "${POSTGRES_TEST_DATABASE_URL:-}" ]]; then
  ...
  started_container="smart-task-board-pg-gate-test4"
  docker rm -f "$started_container" >/dev/null 2>&1 || true
  gate_password="test4-$(date +%s)-$RANDOM"
...
echo "[TEST4 AUTOMATED GATE PASS] Python 3.12, Ruff, PostgreSQL, mini-program and web gates passed."
echo "Remaining release evidence: execute the documented real WeCom device/E2E scenarios with test employees."
