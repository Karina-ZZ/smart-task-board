...
fail() { echo "[TEST6 BLOCKED] $*" >&2; exit 2; }
...
if [[ -z "${POSTGRES_TEST_DATABASE_URL:-}" ]]; then
  ...
  started_container="smart-task-board-pg-gate-test6"
  docker rm -f "$started_container" >/dev/null 2>&1 || true
  gate_password="test6-$(date +%s)-$RANDOM"
...
echo "[TEST6 AUTOMATED GATE PASS] automated gates passed."
