...
@router.get("/users", response_model=list[TaskCreationPersonResponse], summary="List task-creation employee candidates")
def list_people(
    ...
    return [TaskCreationPersonResponse.model_validate(row) for row in TaskCreationPeopleService(session).list_candidates(actor, keyword, limit)]
