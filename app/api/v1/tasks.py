...
from app.api.dependencies import (
    ...
    get_intake_service,
    get_task_decomposition_service,
...
DecompositionService = Annotated[TaskDecompositionService, Depends(get_task_decomposition_service)]
...
def _create_command(request: CreateTaskRequest, actor: str) -> CreateTaskDraftCommand:
    ...
    return CreateTaskDraftCommand(**command)
...
@router.get(
    ...
    response_model=PaginatedTaskOperationLogResponse,
    summary="List task-scoped operation audit logs",
    responses=ERROR_RESPONSES,
)
def list_task_operation_logs(
    ...
    return query_service.list_operation_logs(
        task_id,
        actor,
        limit=limit,
        offset=offset,
...
@router.put(
    "/{task_id}/assignee",
    response_model=TaskActionResponse,
    summary="Reassign a task and return it to pending acceptance",
    responses=ERROR_RESPONSES,
)
def reassign_task(
    task_id: UUID,
    ...
    return _task_response(
        service.reassign_task(
            ...
            request.expected_task_version,
            OPERATION_SOURCE,
            request.new_assignee_employee_no,
            request.reason,
            idempotency_key,
        )
    )
...
@router.post(
    ...
    response_model=TaskActionResponse,
    summary="Accept a task and start AI decomposition",
    responses=ERROR_RESPONSES,
)
def accept_task(
    task_id: UUID,
    ...
    if idempotency_key is None:
        task = service.accept_task(task_id, actor, request.expected_task_version, OPERATION_SOURCE)
    else:
        task = service.accept_task(
            task_id, actor, request.expected_task_version, OPERATION_SOURCE, idempotency_key
        )
    return TaskActionResponse.model_validate(task)
...
@router.get(
    ...
    summary="Get latest AI decomposition attempt",
    responses=ERROR_RESPONSES,
)
def get_decomposition(
    task_id: UUID, actor: Actor, service: DecompositionService
) -> TaskDecompositionResponse:
    return TaskDecompositionResponse.model_validate(service.get_latest(task_id, actor))


@router.post(
    "/{task_id}/decomposition/execute",
    response_model=TaskDecompositionResponse,
    summary="Execute the accepted task's pending AI decomposition",
    responses=ERROR_RESPONSES,
)
def execute_decomposition(
    ...
    result = service.execute(task_id, request.decomposition_id, actor)
    return TaskDecompositionResponse.model_validate(result)


@router.post(
    "/{task_id}/decomposition/retry",
    response_model=TaskActionResponse,
    summary="Retry a failed AI decomposition",
    responses=ERROR_RESPONSES,
)
def retry_decomposition(
    task_id: UUID,
    request: RetryDecompositionRequest,
    actor: Actor,
    service: DecompositionService,
    idempotency_key: Annotated[str | None, Header(alias="Idempotency-Key", max_length=160)] = None,
) -> TaskActionResponse:
    task = service.retry(
        task_id, actor, request.expected_task_version, idempotency_key=idempotency_key
    )
    return TaskActionResponse.model_validate(task)
