...
@router.post(
    "/task-inputs/register",
    response_model=TaskInputRegistrationResponse,
    status_code=status.HTTP_201_CREATED,
    summary="Register a task input without invoking the backend AI provider",
)
def register_task_input(
    ...
    return TaskInputRegistrationResponse.model_validate(
        {
            "input_id": task_input.input_id,
            "input_type": task_input.input_type,
            "raw_text": task_input.raw_text,
...
@router.post(
    "/task-inputs/{input_id}/external-extractions",
    response_model=TaskExtractionResponse,
    status_code=status.HTTP_201_CREATED,
    summary="Validate and store one external ChatService/Qwen extraction round",
)
def record_external_task_extraction(
    input_id: UUID,
    request: ExternalTaskExtractionRequest,
    actor: Actor,
    service: IntakeService,
) -> TaskExtractionResponse:
    result = service.record_external_extraction(actor, input_id, request.model_dump())
...
def suggest_performance_matches(
    task_id: UUID,
    request: PerformanceMatchSuggestRequest,
    actor: Actor,
    service: MetricService,
    limit: Annotated[int, Query(ge=1, le=50)] = 10,
) -> list[Any]:
    return service.suggest_matches(
        actor, task_id, limit, expected_task_version=request.expected_task_version
    )
...
def confirm_performance_match(
    ...
    request: PerformanceMatchConfirmRequest,
    actor: Actor,
    service: MetricService,
) -> Any:
    return service.confirm_match(
        actor, task_id, performance_match_id,
        expected_task_version=request.expected_task_version,
    )


@router.post(
    "/tasks/{task_id}/performance-matches/clear",
    response_model=list[PerformanceMatchResponse],
    summary="Clear the creator-confirmed KPI relation for a task",
)
def clear_performance_match(
    task_id: UUID,
    request: PerformanceMatchConfirmRequest,
    actor: Actor,
    service: MetricService,
) -> list[Any]:
    return service.clear_confirmation(
        actor, task_id, expected_task_version=request.expected_task_version
    )
