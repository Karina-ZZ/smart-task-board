...
def completion_review_allowed_actions(
    ...
    if (
        ...
        and actor in {task.creator_employee_no, current_review.reviewer_employee_no}
    ):
...
def _task_actions(
    ...
    *,
    ...
    elif task.status in {"in_progress", "blocked", "pending_report"}:
    ...
    *,
    ...
    if actor == task.creator_employee_no:
        ...
        if task.status in {
            ...
            actions.extend(["reassign_task", "withdraw_task"])
        if (
...
def _node_actions(
    ...
    *,
    ...
    if task.status not in {"in_progress", "blocked", "pending_report"}:
        return []
    if node.status == "completed" and can_reopen:
        return ["reopen_node"]
    if not can_execute and not can_report:
        return []
    # V1.1 task-level reports/issues belong to the main assignee. Node owners
    # execute nodes but do not get a separate node-reporting channel.
...
class TaskBoardQueryService:
    ...
    def list_tasks(
        ...
        *,
        ...
        quadrant = QUADRANT_ALIASES.get(quadrant, quadrant)
    ...
    def _list_visible_tasks(
        ...
        *,
        ...
        actor_user = self._session.get(User, actor)
        # FR-22 data scope: the default task overview for executives/admins may
        # include tasks granted by their server-side scope. Explicit relation
        # filters still mean exactly created/assigned/participating.
        if (
            relation == "all"
            and actor_user is not None
            and actor_user.status == "active"
            and actor_user.role_type in {"executive", "admin"}
        ):
        ...
        if quadrant is not None:
            latest_priority_at = (
                select(func.max(TaskPriorityScore.calculated_at))
                .where(TaskPriorityScore.task_id == Task.task_id)
                .correlate(Task)
                .scalar_subquery()
            )
            statement = statement.where(
                exists().where(
                    ...
                    TaskPriorityScore.calculated_at == latest_priority_at,
    ...
    def dashboard_summary(self, actor: str) -> dict[str, object]:
        ...
        near_due_end = now + timedelta(days=NEAR_DUE_DAYS)
        completion_period_start = now - timedelta(days=COMPLETION_RATE_PERIOD_DAYS)
        ...
        completion_sample_count, on_time_completion_count = (
            self._tasks.completion_stats_related(
                actor,
                completed_from=completion_period_start,
            )
        )
        on_time_completion_rate = (
            round(on_time_completion_count * 100 / completion_sample_count)
            if completion_sample_count
            else 0
        )
        return {
            ...
            "due_within_3_days_count": self._tasks.count_related(
                actor,
                deadline_from=now,
                deadline_to=near_due_end,
                exclude_completed=True,
            ),
            ...
            "on_time_completion_count": on_time_completion_count,
            "completion_sample_count": completion_sample_count,
            "completion_rate_period_days": COMPLETION_RATE_PERIOD_DAYS,
    ...
    def _priority_items(self, actor: str) -> list[dict[str, object]]:
        ...
        # History is append-only. De-duplicate to the latest score before applying
        # dashboard ranking so one task's older snapshots cannot crowd out others.
        ...
        latest_rows = sorted(
            ...
            key=lambda row: (
                row.sort_rank is None,
                row.sort_rank if row.sort_rank is not None else 10**9,
                -_aware_utc(row.calculated_at).timestamp(),
                str(row.task_id),
            ),
        )[:10]
        return [
    ...
    def _latest_priority(self, task_id: UUID) -> TaskPriorityScore | None:
        return self._session.scalar(
            select(TaskPriorityScore)
            .where(TaskPriorityScore.task_id == task_id)
            .order_by(
                TaskPriorityScore.calculated_at.desc(),
    ...
    def _summary(
        ...
        *,
        ...
        return {
            ...
            "task_weight": task.task_weight,
            "progress_percent": (
                round(sum(node.progress_percent for node in nodes) / len(nodes))
                if nodes
                else (100 if task.status in {"completed", "archived"} else 0)
            ),
            "task_version": task.task_version,
            "creator": {
    ...
    def _current_user_relations(
        ...
        """Project server-authorized task relationships for UI context, not authorization."""
        relations: list[str] = []
        if actor == task.creator_employee_no:
