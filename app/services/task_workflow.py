...
TASK_DECOMPOSING = "decomposing"
TASK_DECOMPOSITION_FAILED = "decomposition_failed"
...
def _task_actual_hours(task: Task, completed_at: datetime) -> Decimal | None:
    """Derive task actual hours from system timestamps; clients never submit it."""
    if task.start_time is None:
        return None
    start = _aware_utc(task.start_time, "start_time")
    seconds = max(0, (completed_at - start).total_seconds())
    return Decimal(str(round(seconds / 3600, 2)))


def _require_completion_reviewer(task: Task, review: TaskCompletionReview, actor: str) -> None:
    """PRD allows the creator or designated reviewer to decide completion."""
    allowed = {task.creator_employee_no, review.reviewer_employee_no}
    if actor not in allowed:
        raise PermissionDeniedError("actor must be the task creator or completion reviewer")
...
def _add_task_notification(
    uow: UnitOfWork,
    task: Task,
    recipient_employee_no: str | None,
    *,
    title: str,
    content: str,
    dedupe_key: str,
    ...
    if not recipient_employee_no:
        return
    uow.session.add(
        Notification(
            task_id=task.task_id,
            recipient_employee_no=recipient_employee_no,
            channel="in_app",
            title=title,
            content=content,
            send_status="pending",
...
def _validate_send_ready_task(uow: UnitOfWork, task: Task) -> None:
    ...
    if task.estimated_hours is not None or task.actual_hours is not None:
        raise BusinessValidationError("hours are not client-writable in the V1.1 create flow")
    if _validate_existing_graph(uow, task.task_id):
        raise BusinessValidationError("creator send flow must not contain task nodes")
    if uow.task_nodes.list_dependencies(task.task_id):
        raise BusinessValidationError("creator send flow must not contain node dependencies")
...
class TaskWorkflowService:
    ...
    def create_change_request(
        ...
        *,
        ...
        with self._uow_factory() as uow:
            cached = self._idempotent_change_request(
                uow,
                task_id,
                actor_employee_no,
                "change_requested",
            ...
            _add_task_notification(
                ...
                title="任务变更待处理",
                content=f"{actor_employee_no}提交了任务变更申请：{normalized_reason}",
                dedupe_key=f"change-request:{request.change_request_id}:submitted",
                now=now,
            )
            uow.commit()
            return task, request
        ...
        *,
        ...
        with self._uow_factory() as uow:
            ...
            _add_task_notification(
                uow,
                task,
                request.requester_employee_no,
                title="任务变更已同意",
                content=normalized_comment or "创建人已同意你的任务变更申请。",
                dedupe_key=f"change-request:{request.change_request_id}:approved",
                now=now,
            )
            uow.commit()
            return task, request
        ...
        *,
        ...
        with self._uow_factory() as uow:
            ...
            _add_task_notification(
                ...
                dedupe_key=f"change-request:{request.change_request_id}:rejected",
                now=now,
            )
            uow.commit()
            return task, request
    ...
    @staticmethod
    def _idempotent_change_request(
        uow: UnitOfWork,
        task_id: UUID,
        actor_employee_no: str,
        action: str,
        ...
        row = uow.session.scalar(
            select(OperationLog).where(
                ...
                OperationLog.operator_employee_no == actor_employee_no,
                OperationLog.action == action,
                OperationLog.object_type == "task_change_request",
        ...
        if not isinstance(row, OperationLog):
            return None
        try:
            request_id = UUID(row.object_id)
        except (TypeError, ValueError):
            return None
        request = uow.task_change_requests.get_by_id(request_id)
        task = uow.tasks.get_by_id(task_id)
        if request is None or task is None or request.task_id != task_id:
            return None
        return task, request
    ...
    def _require_change_requester(
        ...
        if task.status not in {TASK_IN_PROGRESS, "blocked", "pending_report"}:
            raise InvalidStateTransitionError("change requests require an execution-state task")
    ...
    def reassign_task(
        ...
        expected_task_version: int,
        operation_source: str,
        new_assignee_employee_no: str,
        reason: str,
        idempotency_key: str | None = None,
    ) -> Task:
        normalized_reason = _required_text(reason, "reason")
        normalized_source = _required_text(operation_source, "operation_source")
        new_assignee = _required_text(new_assignee_employee_no, "new_assignee_employee_no")
        with self._uow_factory() as uow:
            cached = find_idempotent_task(
                uow.session,
                key=idempotency_key,
                actor=actor_employee_no,
                action="assignee_reassigned",
                task_id=task_id,
            )
            if cached is not None:
                return cached
            ...
            if task.status not in {
                ...
                raise InvalidStateTransitionError(
                    f"assignee reassignment is not allowed from task status {task.status}"
                )
            user = uow.users.get_by_employee_no(new_assignee)
            ...
            for participant in uow.tasks.list_participants(task.task_id):
                if participant.participant_role == "assignee" and participant.is_primary:
                    primary = participant
                    break
            if primary is None:
                raise BusinessValidationError("primary assignee projection is missing")
            primary.employee_no = new_assignee
            primary.confirm_status = PARTICIPANT_CONFIRM_PENDING
            primary.confirmed_at = None
            task.main_assignee_employee_no = new_assignee
            task.status = TASK_PENDING_ACCEPTANCE
            task.accepted_at = None
            task.effective_at = None
            task.decomposition_status = None
            task.latest_decomposition_id = None
            _increment_task(task, now)
            self._cancel_pending_requests(
                uow,
                task,
                actor_employee_no,
                "main assignee was changed",
                now,
            )
            _append_log(
                uow,
                ...
                action_type="assignee_reassigned",
                operator_employee_no=actor_employee_no,
                target_employee_no=new_assignee,
                operation_source=normalized_source,
                reason=normalized_reason,
                now=now,
                request_id=idempotency_key,
            ...
            _add_task_notification(
                ...
                old_assignee,
                title="任务承办人已变更",
                content=f"“{task.task_name}”已更换承办人：{normalized_reason}",
                dedupe_key=f"task:{task.task_id}:reassign:v{task.task_version}:old",
                now=now,
            )
    ...
    def _lifecycle_transition(
        ...
        *,
        ...
        with self._uow_factory() as uow:
            ...
            now = _aware_utc(self._clock(), "clock")
            if previous_status == TASK_DECOMPOSING:
                # A lifecycle write wins over an in-flight AI result. Marking the
                # attempt invalid in the same transaction makes late callbacks harmless.
                from app.services.features.task_decomposition import TaskDecompositionService
            ...
            if to_status in {TASK_CANCELLED, TASK_WITHDRAWN}:
                _add_task_notification(
                    uow,
                    task,
                    task.main_assignee_employee_no,
                    title="任务已取消" if to_status == TASK_CANCELLED else "任务已撤回",
                    content=normalized_reason or "任务状态已变更。",
                    dedupe_key=f"task:{task.task_id}:{to_status}:v{task.task_version}",
                    now=now,
                )
            uow.commit()
            return task
    ...
    def update_task_draft(
        self, task_id: UUID, actor_employee_no: str, expected_task_version: int,
        operation_source: str, changes: Mapping[str, object],
        collaborator_employee_nos: Sequence[str] | None = None,
    ) -> Task:
        editable = {
            "task_name", "task_description", "task_goal", "task_source",
            "main_assignee_employee_no", "report_to_employee_no", "report_to_level",
            "reviewer_employee_no", "department_id", "start_time", "deadline",
            "task_weight", "deliverable", "acceptance_criteria", "is_urgent", "report_cycle",
        }
        unknown = set(changes) - editable
        if unknown:
            raise BusinessValidationError("unsupported draft fields: " + ", ".join(sorted(unknown)))
        with self._uow_factory() as uow:
            task = _lock_task(uow, task_id, expected_task_version)
            ...
            if task.task_name:
                task.task_name = _required_text(task.task_name, "task_name")
            if task.start_time is not None:
                task.start_time = _optional_utc(task.start_time, "start_time")
            if task.deadline is not None:
                task.deadline = _optional_utc(task.deadline, "deadline")
            if task.start_time and task.deadline and task.deadline < task.start_time:
                raise BusinessValidationError("deadline must not precede start_time")
            if task.main_assignee_employee_no != old_assignee:
                if old_assignee:
                    row = uow.tasks.find_participant(task.task_id, old_assignee, "assignee")
                    if row is not None:
    ...
    def submit_for_confirmation(
        ...
        with self._uow_factory() as uow:
            ...
            _validate_send_ready_task(uow, task)
            now = _aware_utc(self._clock(), "clock")
    ...
    def accept_task(
        ...
        """Compatibility facade for DEV-09 acceptance; never activates the task directly."""
        from app.services.features.task_decomposition import TaskDecompositionService
    ...
    def submit_completion(
        ...
        with self._uow_factory() as uow:
            cached = self._idempotent_completion_review(
                uow,
                task_id,
                actor_employee_no,
            ...
            if cached is not None:
                return cached
            task = _lock_task(uow, task_id, expected_task_version)
            if task.status not in {TASK_IN_PROGRESS, "blocked"}:
                raise InvalidStateTransitionError(
                    "submit_completion requires an in-progress or blocked task"
                )
            previous_status = task.status
            _require_actor(
                actor_employee_no,
            ...
            _add_task_notification(
                ...
                reviewer,
                title="任务等待验收",
                content=f"“{task.task_name}”已提交完成，请进行验收。",
                dedupe_key=f"task:{task.task_id}:completion-review:{review.review_round}",
                now=now,
            )
            uow.commit()
            return task, review
    ...
    def approve_completion(
        ...
        with self._uow_factory() as uow:
            ...
            _require_completion_reviewer(task, review, actor_employee_no)
            nodes = _validate_existing_graph(uow, task.task_id)
            self._require_completed_nodes(nodes)
            if uow.task_issues.has_non_closed(task.task_id):
                raise OpenTaskIssueConflictError(
                    "all task issues must be closed before approving completion"
                )
            if uow.task_archives.get_by_task_id(task.task_id) is not None:
                raise InvalidStateTransitionError("task already has an archive record")
            now = _aware_utc(self._clock(), "clock")
            task.status = TASK_COMPLETED
            task.completed_at = now
            task.actual_hours = _task_actual_hours(task, now)
            _increment_task(task, now)
            review.review_status = "approved"
            review.review_result = "approved"
            review.reviewed_at = now
            # Status-log persistence flushes the session. Keep the review row
            # valid at every flush inside the larger approval/archive transaction.
            ...
            archive = TaskArchive(
                ...
                source_status_snapshot=TASK_COMPLETED,
                summary=review.deliverable_summary or task.task_name,
                search_keywords=[],
                review_result="approved",
                risk_points=[],
                reusable_template=None,
                actual_hours_total=task.actual_hours,
                archived_by_employee_no=actor_employee_no,
                archived_at=now,
            )
            uow.task_archives.add(archive)
            ...
            _add_task_notification(
                ...
                task.main_assignee_employee_no,
                title="任务已验收归档",
                content=f"“{task.task_name}”已验收通过并自动归档。",
                dedupe_key=f"task:{task.task_id}:archived:v{task.task_version}",
                now=now,
            )
            uow.commit()
            return task, review
    ...
    def reject_completion(
        ...
        with self._uow_factory() as uow:
            ...
            _add_task_notification(
                ...
                task.main_assignee_employee_no,
                title="任务验收退回",
                content=f"“{task.task_name}”验收未通过：{normalized_reason}",
                dedupe_key=f"task:{task.task_id}:completion-rejected:{review.review_round}",
                now=now,
            )
            uow.commit()
            return task, review
    ...
    @staticmethod
    def _idempotent_completion_review(
        uow: UnitOfWork,
        task_id: UUID,
        actor_employee_no: str,
        action: str,
        idempotency_key: str | None,
    ) -> tuple[Task, TaskCompletionReview] | None:
        if not idempotency_key:
            return None
        row = uow.session.scalar(
            select(OperationLog)
            .where(
                ...
                OperationLog.operator_employee_no == actor_employee_no,
                OperationLog.action == action,
                OperationLog.object_type == "completion_review",
                OperationLog.result == "success",
            )
        ...
        if not isinstance(row, OperationLog):
            return None
        try:
            review_id = UUID(row.object_id)
        except (TypeError, ValueError):
            return None
        review = uow.task_completion_reviews.get_by_id(review_id)
        task = uow.tasks.get_by_id(task_id)
        if review is None or task is None or review.task_id != task_id:
            return None
        return task, review
