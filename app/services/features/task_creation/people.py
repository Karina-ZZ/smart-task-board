...
Responsibilities: return active employees the actor may assign, enriched with department
and latest workload.
...
class TaskCreationPeopleService:
    ...
    def list_candidates(
        self, actor: str, keyword: str | None = None, limit: int = 100
        ...
        if keyword and keyword.strip():
            ...
            statement = statement.where(
                User.name.ilike(pattern) | User.employee_no.ilike(pattern)
            )
        policy = PermissionScopeService(self.session)
        users = [
            user
            for user in self.session.scalars(statement.limit(300)).all()
            if policy.can_assign_employee(actor, user.employee_no)
        ]
        rows = []
        for user in users:
            snapshot = self.session.scalar(
                select(WorkloadSnapshot)
                .where(WorkloadSnapshot.employee_no == user.employee_no)
        ...
        rows.sort(
            key=lambda item: (
                item["workload_score"] is None,
                item["workload_score"] or 999,
                item["name"],
                item["employee_no"],
            )
        )
