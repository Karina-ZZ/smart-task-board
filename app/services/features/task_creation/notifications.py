...
def add_pending_accept_notification(session: Session, task: Task, *, created_at: datetime) -> Notification:
    ...
    row = Notification(
        task_id=task.task_id, recipient_employee_no=recipient, channel="in_app",
        title="新任务待接受", content=f"“{task.task_name}”等待你接受。",
        send_status="pending", retry_count=0,
        dedupe_key=f"task:{task.task_id}:pending_accept:v{task.task_version}", created_at=created_at,
    )
    session.add(row)
    return row
