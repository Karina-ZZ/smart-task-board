...
from app.models import Notification, Task, TaskIssue, TaskNode
...
class TaskIssueService:
    ...
    def create(self, command: CreateTaskIssueCommand) -> TaskIssue:
        ...
        with self._uow_factory() as uow:
            ...
            if task.status not in {TASK_IN_PROGRESS, "blocked", "pending_report"}:
                raise InvalidStateTransitionError("task issue requires an execution state")
            ...
            uow.session.add(
                Notification(
                    ...
                    send_status="pending",
                    retry_count=0,
                    dedupe_key=f"task-issue:{issue.issue_id}",
                    created_at=now,
                )
            )
            _append_log(
    ...
    def transition(
        ...
        with self._uow_factory() as uow:
            ...
            if task.status not in {TASK_IN_PROGRESS, "blocked", "pending_report"}:
                raise InvalidStateTransitionError("task issue action requires an execution state")
            ...
            if (
                task.status == "blocked"
                and target_status in {"resolved", "rejected", "closed"}
                and uow.task_issues.has_active_blocker_for_task(task.task_id) is not True
            ):
    ...
    def _require_reporter(
        ...
        if employee_no == task.main_assignee_employee_no:
            return
        raise PermissionDeniedError("only the main assignee may report task issues")
