...
class TaskInputCreateRequest(StrictSchema):
    ...
    source_channel: Literal["web", "api", "wecom", "wechat_miniprogram"] = "api"
...
class TaskInputRegistrationResponse(StrictSchema):
    input_id: UUID
    input_type: str
    raw_text: str | None
    asr_text: str | None
...
class ExternalTaskExtractionRequest(StrictSchema):
    task_draft: dict[str, Any]
    missing_fields: list[str] = Field(default_factory=list, max_length=50)
    low_confidence_fields: list[str] = Field(default_factory=list, max_length=50)
    confirm_questions: list[str] = Field(default_factory=list, max_length=10)
...
class PerformanceMatchSuggestRequest(StrictSchema):
    expected_task_version: int = Field(
        ge=1, validation_alias=AliasChoices("expected_task_version", "expectedTaskVersion")
    )


class PerformanceMatchConfirmRequest(StrictSchema):
    expected_task_version: int = Field(
        ge=1, validation_alias=AliasChoices("expected_task_version", "expectedTaskVersion")
    )
