...
class TaskRepository:
    ...
    def completion_stats_related(
        self,
        employee_no: str,
        *,
        completed_from: datetime,
    ) -> tuple[int, int]:
        """Return eligible completions and the subset completed by deadline."""
        statement = select(
            func.count(Task.task_id),
            func.count(Task.task_id).filter(Task.completed_at <= Task.deadline),
        ).where(
            self._related_predicate(employee_no),
            Task.status.in_(("completed", "archived")),
            Task.completed_at.is_not(None),
            Task.deadline.is_not(None),
            Task.completed_at >= completed_from,
        )
        total, on_time = self.session.execute(statement).one()
        return int(total or 0), int(on_time or 0)
