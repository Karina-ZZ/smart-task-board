...
class Task(Base):
    ...
    decomposition_status: Mapped[str | None] = mapped_column(
        String,
        nullable=True,
        index=True,
    )
    latest_decomposition_id: Mapped[UUID | None] = mapped_column(
        Uuid(as_uuid=True),
        ForeignKey("task_decomposition_records.decomposition_id", ondelete="RESTRICT"),
        nullable=True,
        index=True,
    )
    ...
    change_requests: Mapped[list[TaskChangeRequest]] = relationship(
        back_populates="task",
        foreign_keys="TaskChangeRequest.task_id",
        order_by=(
            "(TaskChangeRequest.created_at, "
            "TaskChangeRequest.change_request_id)"
        ),
    )
    decomposition_records: Mapped[list[TaskDecompositionRecord]] = relationship(
        back_populates="task",
        foreign_keys="TaskDecompositionRecord.task_id",
        order_by=(
