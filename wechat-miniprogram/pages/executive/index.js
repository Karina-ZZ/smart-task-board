...
Page({
  ...
  onShow() { this.load(); },
  load() { Promise.all([api.currentUser(), api.notifications("all")]).then(([user, notices]) => { if (!access.canAccessExecutive(user)) { this.setData({ user, denied: true, loading: false, unread: notices.filter((item) => item.actionRequired).length }); return; } return api.executiveOverview().then((overview) => this.setData({ user, overview: { ...overview, members: (overview.members || []).map((member) => ({ ...member, avatarText: member.name ? member.name.slice(-1) : "员" })) }, loading: false, denied: false, unread: notices.filter((item) => item.actionRequired).length })); }).catch((error) => { wx.showToast({ title: error.message, icon: "none" }); this.setData({ loading: false }); }); },
