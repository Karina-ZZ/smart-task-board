...
export function suggestPerformanceMatches(
  taskId: string,
  taskVersion: number,
  limit = 10,
  ...
  return apiRequest<PerformanceMatch[]>(
    ...
    { method: "POST", body: JSON.stringify({ expectedTaskVersion: taskVersion }) },
  );
}
...
export function clearPerformanceMatch(taskId: string, taskVersion: number): Promise<PerformanceMatch[]> {
  return apiRequest<PerformanceMatch[]>(`/api/v1/tasks/${taskId}/performance-matches/clear`, {
    method: "POST",
    body: JSON.stringify({ expectedTaskVersion: taskVersion }),
  });
}
