/**
 * Test14 F1: create/send real data BEFORE testing the creator's workbench.
...
async function prepareSentTask(request: APIRequestContext) {
  ...
  expect(process.env.STB_TEST14_ALLOW_TEST_WRITES).toBe("1");
  ...
  const taskName = `Test14-${taskId.slice(0, 8)}`;
  const now = new Date();
  ...
  for (const action of ["submit-for-confirmation", "confirm-and-send"]) {
    const response = await request.post(`${apiBase}/api/v1/tasks/${taskId}/actions/${action}`, {
      headers: { ...headers, "Idempotency-Key": `test14-${taskId}-${action}` },
      data: { expected_task_version: version },
    });
    expect(response.status()).toBe(200);
    const result = await response.json();
    version = result.task_version;
    if (action === "confirm-and-send") expect(result.status).toBe("pending_acceptance");
  }
...
test.describe("Test14 creator send then workbench", () => {
  test.skip(!enabled, "Requires explicitly isolated real services; run the Test14 gate");

  test("shows the newly sent task after login, detail return and reload", async ({ page, request }) => {
